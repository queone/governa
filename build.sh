...
#
# Shell port that retires the Go tools governa-buildtool (cmd/build),
# governa-reltool (cmd/rel), and internal/preptool (cmd/prep). All logic lives
# here as focused functions behind a guarded main, so tests/run.sh can source
# this file to unit-test individual functions. Targets Bash 3.2+ (macOS system
# bash): no associative arrays, mapfile, ${var^^}, or &>>.
#
# Dispatch:
...
# ════════════════════════════════════════════════════════════════════════════
# Build pipeline (retires cmd/build + governa-buildtool)
# ════════════════════════════════════════════════════════════════════════════
...
EOF
  ...
  # governa-source-only self-test, only on full (no-target) builds — preserves
  # target isolation. Silent on success. Skipped in subprocesses (recursion
  # guard) and in the consumer overlay.
  if [ "${#targets[@]}" -eq 0 ] && [ -z "${GOVERNA_BUILD_TEST:-}" ] &&
    [ -z "${GOVERNA_OVERLAY_BUILD:-}" ] && [ -f tests/run.sh ]; then
    local selftest_out selftest_rc=0
    selftest_out=$(GOVERNA_BUILD_TEST=1 bash tests/run.sh 2>&1) || selftest_rc=$?
    if [ "$selftest_rc" -ne 0 ]; then
      printf '%s\n' "$selftest_out" | write_indented
      printf 'build.sh self-test (tests/run.sh) failed\n' >&2
      return 1
    fi
  fi
...
EOF
  ...
  find "$dir" \( -name .git -o -name node_modules -o -name vendor \) -prune -o \
...
# ── example render + DOC smoke (governa-source-only PostInstallHook port) ─────
_render_and_validate_examples() { # $1=verbose
  local verbose="$1"
  printf '\n%s\n' "$(yel5 '==> Render example repos and validate')"
  local ex_dir=/tmp/governa-examples
  rm -rf "$ex_dir"
  local ex_code="$ex_dir/code" ex_doc="$ex_dir/doc"

  local flavor dir module
  for flavor in code doc; do
    if [ "$flavor" = code ]; then
  ...
  local vet_out
  if ! vet_out=$( cd "$ex_code" && go vet ./... 2>&1 ); then
    printf '%s\n' "$vet_out" | write_indented
    printf 'go vet failed on rendered code example\n' >&2
    return 1
  fi
  printf '    %s\n' 'go vet examples/code: ok'
  local ta=(test); [ "$verbose" = 1 ] && ta+=(-v); ta+=(./...)
  ( cd "$ex_code" && run_streaming_in_dir "$ex_code" grn5 go "${ta[@]}" )
  local overlay_out overlay_rc=0
  overlay_out=$( cd "$ex_code" && NO_COLOR=1 ./build.sh 2>&1 ) || overlay_rc=$?
  if [ "$overlay_rc" -ne 0 ]; then
    printf '%s\n' "$overlay_out" | write_indented
    printf 'rendered code example build failed\n' >&2
    return 1
  fi
  ...
  printf '\n%s\n' "$(yel5 '==> Smoke test DOC overlay (no-go.mod adoption)')"
  local smoke=/tmp/governa-doc-smoke
  rm -rf "$smoke"
  run_streaming grn5 go run ./cmd/governa render-canon --flavor doc "$smoke"
  if [ -f "$smoke/go.mod" ]; then
...
# ════════════════════════════════════════════════════════════════════════════
# Release path (retires cmd/rel + governa-reltool)
# ════════════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════════════
# Release-prep path (retires cmd/prep + internal/preptool)
# ════════════════════════════════════════════════════════════════════════════
...
rel_main() {
  ...
  # governa-source-only: re-install governa AFTER the release commit so the
  # installed binary's vcs.revision matches the new HEAD. Omitted from the
  # consumer overlay build.sh.tmpl.
  go install ./cmd/governa
  local installed_rev head_rev
  installed_rev=$(go version -m "$(go env GOPATH)/bin/governa$(go env GOEXE)" | awk -F= '/vcs.revision=/{print $2}')
  head_rev=$(git rev-parse HEAD)
  if [ "$installed_rev" != "$head_rev" ]; then
    printf 'ERROR: installed binary'\''s vcs.revision (%s) does not match HEAD (%s)\n' "$installed_rev" "$head_rev" >&2
    return 1
  fi
}
...
# Guarded entrypoint: run only when executed, not when sourced (so tests/run.sh
# can source this file to unit-test functions).
