...
- IE7: extract `internal/color` → `github.com/queone/governa-color` (leaf-clean, first extraction; sets family precedent) → [docs/ac97-color-extraction.md](docs/ac97-color-extraction.md)
- IE8: extract `internal/reltool` → `github.com/queone/governa-reltool` (verify leaf-cleanness via convention-coupling test) → [docs/ac98-reltool-extraction.md](docs/ac98-reltool-extraction.md)
- IE9: extract `internal/buildtool` → `github.com/queone/governa-buildtool` (light coupling; small config surface or stay template per the test) → [docs/ac99-buildtool-extraction.md](docs/ac99-buildtool-extraction.md)
