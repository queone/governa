...
2. **Application — pure-canon files.** Whole-file overwrite from canon. Typical examples: `cmd/rel/main.go`, `docs/roles.md`, `docs/ac-template.md`, `docs/README.md`.
...
5. **Boundary-aware mixed-content files.** Files with a documented canon-above/local-below boundary that adopters merge by hand: `AGENTS.md` (boundary `## Project Rules`), `docs/development-guidelines.md` and `docs/editing-guidelines.md` (boundary `## Project Practices`). Files without a named canon boundary (e.g., `README.md`, `CHANGELOG.md`, `plan.md`) are handled by the expected-divergence registry or preserve markers — see `docs/drift-scan.md` `## Expected-divergence registry` and `## Preserve-marker phrase set`.
