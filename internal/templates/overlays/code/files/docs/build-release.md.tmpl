...
4. **Detect version targets.** Scans `cmd/*/main.go` for `programVersion`, `TEMPLATE_VERSION` and `internal/templates/version.go` (each presence-gated). The `programVersion` regex matches both inline (`const programVersion = "..."`) and grouped (`const ( ... programVersion = "..." ... )`) forms. Safe auto-detect filter: 1 `programVersion` target → bump (single-utility repo, repo-tracked). >1 targets → drop all and log a multi-utility warning (per-utility-independent default; each utility owns its own version per its own AC). The skip prevents clobbering independent per-utility SemVers in multi-utility repos.
