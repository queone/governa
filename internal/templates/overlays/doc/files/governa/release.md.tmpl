...
This repo uses `./rel.sh vX.Y.Z "release message"` to cut releases. `rel.sh` is a thin Bash dispatcher that runs `go run ./cmd/rel/main.go ./cmd/rel/color.go "$@"` (file-list form, module-mode-independent — no `go.mod` required). The release tool shows `git status --short`, lists every git step it will execute, and prompts for interactive confirmation. On approval it orchestrates `git add → commit → tag → push tag → push branch`.
...
5. **Delete completed AC files.** Remove `governa/ac<N>-*.md` files for ACs shipping in this release.
