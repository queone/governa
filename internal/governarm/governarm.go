...
func buildStub(acNum int, canonVersion, diffsRel string, inScope, outOfScope, review []routing) string {
	...
	fmt.Fprint(&b, "\n## Summary\n\n")
	fmt.Fprintln(&b, "This AC was emitted by `governa rm`. Implement only after the Director resolves the review items below.")
	fmt.Fprint(&b, "\n## Objective Fit\n\n")
	fmt.Fprintln(&b, "1. **Outcome.** Extricate Governa canon without deleting consumer-owned content.")
	fmt.Fprintln(&b, "2. **Priority.** Director requested Governa removal from this consumer repo.")
	fmt.Fprintf(&b, "3. **Dependencies.** Emitted by governa rm against canon %s.\n", canonVersion)
	fmt.Fprint(&b, "\n## In Scope\n\n")
	writeRoutingList(&b, inScope)
	fmt.Fprint(&b, "\n## Out Of Scope\n\n")
	writeRoutingList(&b, outOfScope)
	fmt.Fprint(&b, "\n## Implementation Notes\n\n")
	fmt.Fprintf(&b, "- Use `%s` for hunk-level guidance on hybrid files.\n", diffsRel)
	fmt.Fprintln(&b, "- Do not auto-delete Director Review files until the Director chooses their routing.")
	fmt.Fprint(&b, "\n## Acceptance Tests\n\n")
	fmt.Fprintln(&b, "**AT1** [Automated] [Pre-release gate] — Removed files listed under `## In Scope` no longer exist.")
	fmt.Fprintln(&b, "**AT2** [Manual] [Pre-release gate] — Director confirms every `## Director Review` file was routed exactly as decided.")
	fmt.Fprint(&b, "\n## Documentation Updates\n\n")
	fmt.Fprintln(&b, "- Update repo-local docs only if the Director chooses to preserve Governa-derived explanations after removal.")
	fmt.Fprint(&b, "\n## Director Review\n\n")
	if len(review) == 0 {
		fmt.Fprintln(&b, "`None`.")
	} else {
