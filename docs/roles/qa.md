...
- **Calibrate verbosity to findings density.** When the round's verdict is "no blockers" with no director-attention items, render findings as one-liners (location + fix) and omit observations and the five-field terminator. Reserve the full structure for rounds with blockers, residual risks, or director-attention items (scope calls, version classification, design trade-offs).
- **Skip the enumerated "Verified against code" block when every check passed.** Replace with a one-line pass signal (e.g., "verified core claims against code") at the top of the findings. Keep a shortened verification block only when one or more checks surface something worth flagging inline. This rule applies independently of verdict and director-attention state — passing checks add no load-bearing content.
