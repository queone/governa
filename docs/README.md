# Docs

Index for repo-level workflow and maintenance docs.

- [governance-model.md](governance-model.md): governance model
- [operator-contract-rationale.md](operator-contract-rationale.md): why Governa uses a session-entry contract and how it's enforced
- [drift-scan.md](drift-scan.md): consumer-run drift detection and adoption-AC emission
- [build-release.md](build-release.md): canonical local build and release rules
- [development-cycle.md](development-cycle.md): working cycle and artifact expectations
- [development-guidelines.md](development-guidelines.md): durable engineering guidance for any agent or contributor
- [overlay-scope.md](overlay-scope.md): `CODE` versus `DOC` boundaries
- [critique-protocol.md](critique-protocol.md): AC critique round structure and integration
- [ac-template.md](ac-template.md): acceptance-criteria template
- [roles.md](roles.md): two-role model (Operator, Director)
- `ac<N>-<slug>.md`: acceptance-criteria docs (working AC files; `ac-template.md` is the keeper)
