...
- `drift-scan <repo-path>`: run `governa drift-scan <repo-path>`. Tool emits two report files at the target root. Hand off in one line and stop. Do not author or modify any AC in the target repo.
