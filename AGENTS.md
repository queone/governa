...
- Track forward-looking work in `plan.md` only via IE entries (pre-rubric IEs or AC-pointers, per `plan.md`'s docstring). No top-level work-tracking sections (e.g. `## <Name> Track`). For multi-AC tracks, draft skeleton AC stubs first, then register one AC-pointer IE per stub. Remove AC-pointer IEs when their AC ships and the file is deleted at release prep — `plan.md` is not a historical record. Director may skip the IE entirely when an AC is drafted and implemented inside a single cycle.
- `drift-scan <repo-path>`: run `governa drift-scan <repo-path>`. Tool emits two report files at the target root. Report a two-line handoff ("report produced; governa's part is done; <repo-name> agent takes it from here") and stop.
