...
governa is a template repo that bootstraps governance into new repositories and helps existing ones adopt it with minimal disruption. Template improvements happen out-of-band via agent review of consumer repos; there is no CLI subcommand for it.
...
- Do not start release-prep bookkeeping early. Only begin when the user explicitly asks to prep for release.
- Follow the Pre-Release Checklist in `docs/build-release.md` for the full procedure.
